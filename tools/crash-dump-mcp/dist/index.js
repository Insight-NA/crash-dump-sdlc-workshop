...
// Tool: parse_minidump
// ─────────────────────────────────────────────────────────────────────────────
server.tool("parse_minidump", "Open and parse a Windows minidump (.dmp) file. Returns exception code, faulting thread, module count, and process info. Must be called before other tools.", {
...
// Tool: get_call_stack
// ─────────────────────────────────────────────────────────────────────────────
server.tool("get_call_stack", "Get the call stack for a specific thread in the loaded dump. Returns frames with module, function, source file, and line number.", {
    ...
    if (!cdb)
        throw new Error("No dump loaded. Call parse_minidump first.");
    if (threadId !== undefined) {
        await cdb.execute(`~${threadId}s`);
    }
    const raw = await cdb.execute(`kP ${maxFrames}`);
    const frames = parseCallStack(raw);
    return {
...
// ─────────────────────────────────────────────────────────────────────────────
server.tool("resolve_symbols", "Resolve an address to a symbol (function name, source file, line number) using loaded PDB symbols.", {
...
// Tool: analyze_crash
// ─────────────────────────────────────────────────────────────────────────────
server.tool("analyze_crash", "Run automated crash analysis (equivalent to WinDbg '!analyze -v'). Returns exception details, faulting frame, root cause hypothesis, and call stack.", {}, async () => {
    if (!cdb)
        throw new Error("No dump loaded. Call parse_minidump first.");
...
// ─────────────────────────────────────────────────────────────────────────────
server.tool("get_thread_context", "Get register state and thread context for a specific thread.", {
...
// Tool: list_modules
// ─────────────────────────────────────────────────────────────────────────────
server.tool("list_modules", "List all loaded modules (DLLs/executables) with PDB match status. Use to verify symbols are loaded correctly before analysis.", {}, async () => {
    if (!cdb)
        throw new Error("No dump loaded. Call parse_minidump first.");
...
// ─────────────────────────────────────────────────────────────────────────────
server.tool("get_memory_region", "Read a memory region from the dump at a specific address. Returns hex + ASCII dump. Useful for inspecting heap corruption.", {
...
// Helpers
// ─────────────────────────────────────────────────────────────────────────────
function mapExceptionCode(code) {
    const map = {
...
function extractProcessName(vertarget) {
    const match = vertarget.match(/Image:\s+(.+?)(?:\s|$)/i);
    return match?.[1] ?? "unknown";
}
function parseSymbolOutput(lnOutput, address) {
...
function parseModuleList(raw) {
    ...
    for (const line of lines) {
        const match = line.match(/([0-9a-f`]+)\s+([0-9a-f`]+)\s+(\w+)\s+.*?\(([^)]*)\)/i);
        if (match) {
            modules.push({
                name: match[3],
                path: "",
...
function extractAsciiColumn(hexDump) {
    return hexDump
        .split("\n")
        .map((line) => {
