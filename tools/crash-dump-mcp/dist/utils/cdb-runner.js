 ...
 */
export function parseCallStack(raw) {
    const frames = [];
    const lines = raw.split("\n").filter((l) => l.trim().length > 0);
    // Skip header lines (Child-SP, RetAddr, etc.)
    const dataLines = lines.filter((l) => !l.startsWith("Child-SP") && !l.startsWith(" #") && !l.startsWith("---"));
    let frameNum = 0;
    for (const line of dataLines) {
        // Format: RetAddr : Module!Function+Offset [SourceFile @ Line]
        const match = line.match(/([0-9a-f`]+)\s+([0-9a-f`]+)\s+(\w+)!(.+?)(?:\+0x([0-9a-f]+))?(?:\s+\[(.+?)\s*@\s*(\d+)\])?/i);
        if (match) {
            frames.push({
