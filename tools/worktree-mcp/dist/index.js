...
// ─────────────────────────────────────────────────────────────────────────────
server.tool("create_worktree", "Create a new git worktree for an isolated fix branch. Each resolution attempt gets its own worktree so fixes can be developed and tested in parallel.", {
    ...
    const { stdout, stderr } = await exec("git", ["worktree", "add", "-b", branchName, worktreePath, baseBranch], { cwd: absRepo });
    return {
        content: [{
                type: "text",
                text: JSON.stringify({
...
// Tool: list_worktrees
// ─────────────────────────────────────────────────────────────────────────────
server.tool("list_worktrees", "List all active git worktrees for the repository. Shows which fix branches are currently active.", {
    repoPath: z.string().describe("Path to the main git repository"),
}, async ({ repoPath }) => {
    const absRepo = resolve(repoPath);
...
// ─────────────────────────────────────────────────────────────────────────────
server.tool("remove_worktree", "Remove a git worktree after a fix is merged or abandoned. Cleans up the branch and working directory.", {
    repoPath: z.string().describe("Path to the main git repository"),
...
// Tool: cmake_build
// ─────────────────────────────────────────────────────────────────────────────
server.tool("cmake_build", "Run a CMake configure + build in a worktree. Uses the project's CMakePresets.json. Returns build status, errors, and warnings.", {
    ...
    try {
        ...
        // Build
        const { stdout, stderr } = await exec("cmake", ["--build", "--preset", preset], { cwd: absPath, timeout: 300000 });
        // Parse warnings and errors from output
        const combined = stdout + stderr;
        for (const line of combined.split("\n")) {
            if (/error\s*(C|LNK)\d+/i.test(line))
                errors.push(line.trim());
            else if (/warning\s*C\d+/i.test(line))
...
// Tool: run_tests
// ─────────────────────────────────────────────────────────────────────────────
server.tool("run_tests", "Run CTest in a worktree build directory. Returns pass/fail counts and names of failed tests. Use after cmake_build to validate a fix.", {
...
}, async ({ worktreePath, preset, filter }) => {
    const absPath = resolve(worktreePath);
    const start = Date.now();
...
// Tool: worktree_diff
// ─────────────────────────────────────────────────────────────────────────────
server.tool("worktree_diff", "Get the git diff of changes in a worktree relative to its base branch. Use to review a fix before the HITL gate.", {
    worktreePath: z.string().describe("Path to the worktree"),
    baseBranch: z.string().optional().default("main").describe("Base branch to diff against"),
}, async ({ worktreePath, baseBranch }) => {
    const absPath = resolve(worktreePath);
    const { stdout } = await exec("git", ["diff", `${baseBranch}...HEAD`, "--stat"], { cwd: absPath });
    const { stdout: fullDiff } = await exec("git", ["diff", `${baseBranch}...HEAD`], { cwd: absPath });
    return {
        content: [{
                type: "text",
                text: JSON.stringify({
